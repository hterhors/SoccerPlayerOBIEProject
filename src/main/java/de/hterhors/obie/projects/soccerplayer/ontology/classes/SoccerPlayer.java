...
static class SoccerPlayerIndividual extends AbstractIndividual {
 ...
 return this;	}public InvestigationRestriction investigationRestriction;	final static public String ONTOLOGY_NAME = "http://dbpedia.org/ontology/SoccerPlayer";
...
for (int j = 0; j < soccerPlayer.getBirthPlaces().size(); j++) {if (soccerPlayer.getBirthPlaces().get(j) != null) {birthPlaces.add((IPlace) IOBIEThing.getCloneConstructor(soccerPlayer.getBirthPlaces().get(j).getClass()).newInstance(soccerPlayer.getBirthPlaces().get(j)));} else {birthPlaces.add(null);}}
if(soccerPlayer.getBirthYear()!=null)this.birthYear = new BirthYear((BirthYear)soccerPlayer.getBirthYear());
this.characterOffset = soccerPlayer.getCharacterOffset();
this.characterOnset = soccerPlayer.getCharacterOnset();
if(soccerPlayer.getDeathYear()!=null)this.deathYear = new DeathYear((DeathYear)soccerPlayer.getDeathYear());
for (int j = 0; j < soccerPlayer.getPositionAmerican_football_positions().size(); j++) {if (soccerPlayer.getPositionAmerican_football_positions().get(j) != null) {positionAmerican_football_positions.add((IAmerican_football_positions) IOBIEThing.getCloneConstructor(soccerPlayer.getPositionAmerican_football_positions().get(j).getClass()).newInstance(soccerPlayer.getPositionAmerican_football_positions().get(j)));} else {positionAmerican_football_positions.add(null);}}
for (int j = 0; j < soccerPlayer.getTeamSoccerClubs().size(); j++) {if (soccerPlayer.getTeamSoccerClubs().get(j) != null) {teamSoccerClubs.add((ISoccerClub) IOBIEThing.getCloneConstructor(soccerPlayer.getTeamSoccerClubs().get(j).getClass()).newInstance(soccerPlayer.getTeamSoccerClubs().get(j)));} else {teamSoccerClubs.add(null);}}
...
return this;}
	/***/
@Override
	public SoccerPlayer addPositionAmerican_football_positions(IAmerican_football_positions american_football_positions){
...
return this;}
	/***/
@Override
	public SoccerPlayer setPositionAmerican_football_positions(List<IAmerican_football_positions> american_football_positions){
		if(american_football_positions==null){throw new IllegalArgumentException("Can not set list objects to null.");}this.positionAmerican_football_positions = american_football_positions;
return this;}
	/***/
@Override
...
@Override
public String toString(){
return "SoccerPlayer [individual="+individual+",investigationRestriction="+investigationRestriction.summarize()+",birthPlaces="+birthPlaces+",birthYear="+birthYear+",characterOffset="+characterOffset+",characterOnset="+characterOnset+",deathYear="+deathYear+",positionAmerican_football_positions="+positionAmerican_football_positions+",serialVersionUID="+serialVersionUID+",teamSoccerClubs="+teamSoccerClubs+",textMention="+textMention+"]";}
